...
TaxIDs are found in the following file:

* `data/chopped/ids.txt`
...
# Read in refseq info
ids <- readLines('../../data/chopped/ids.txt')
...
class_df <- left_join(class_df, organisms_df, by = "refseqID") %>% 
  rename(seq_id = "refseqID")

class_df %>%
  select(record, seq_id, length, actual, description) %>%
  unique()
```
...
class_df <- left_join(class_df, taxonomy, by = "seq_id")

organisms_df <- left_join(organisms_df, taxonomy, by = c("refseqID" = "seq_id"))
```
...
for (tool_i in levels(two_class$tool)) {
  for (length_i in levels(two_class$length)) {
    tool_df <- two_class %>%
      filter(tool == tool_i) %>%
      filter(length == length_i)
