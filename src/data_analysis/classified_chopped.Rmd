...
Taxonomy information is gathered from:
...
raw_class_df <-
  read.csv('../../data/classified_chopped_small/combined_out/combined_2.csv')
...
# Read in taxonomy
raw_taxonomy_df <-
  read.csv('../../data/refseq_info/taxonomy.csv')
...
Join the taxonomy information to the dataframe

```{r add organisms, echo = FALSE}
taxonomy_df <- raw_taxonomy_df %>%
  select(-kingdom) %>% 
  unique()
...
class_df <- left_join(class_df, taxonomy_df, by = "seq_id")
...
viral %>%
  mutate(order = if_else(is.na(order), "unknown", order)) %>%
  group_by(order) %>%
  count() %>%
  mutate(num_frags = n / length(levels(viral$tool))) %>%
  select(-n) %>%
