...
calc_distance = function(df) {
  df %>%
    ...
    pivot_wider(
      id_cols = c(profile, profile_version, origin),
      names_from = taxonomy_id,
      values_from = proportion
    ) %>%
    ungroup() %>%
    select(-profile, -profile_version, -origin) %>%
    vegdist(na.rm = TRUE)
}
```
...
bray_df <- bray_df %>%
  mutate(
    origin = case_when(
      grepl("ocean", profile) ~ "Ocean",
      grepl("adult", profile) ~ "Human Gut",
      grepl("infant", profile) ~ "Human Gut",
      grepl("soil", profile) ~ "Soil",
      grepl("skin", profile) ~ "Human Skin"
    )
  )
...
```{r, echo=FALSE}
bray_df %>% 
  ggplot(aes(x = distance, y = profile)) +
...
profiles %>%
  ...
  group_by(origin, profile, superkingdom) %>%
...
```{r, echo=FALSE}
contig_tax_summary %>% 
  ggplot(aes(x = model, y = prop_phage, color = origin, fill = origin)) +
  facet_wrap(~ profile) +
  geom_col() +
  scale_fill_hurwitz('main') +
  scale_color_hurwitz('main') +
  labs(x = "InSilicoSeq Error Model",
       y = "Phage Proportion of Contigs",
       color = "Origin",
       fill = "Origin",
       title = "Phage Presence in Assemblies")
```
